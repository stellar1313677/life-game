...
SDD v2 §15.2 的 MVP 範圍，加上 §15.3 P1/P2/P3 後續擴充順序，目前**全部完成**：

| 模組 | 狀態 |
|---|---|
| 分支 | **三個分支全部開放可選**：現代／科技、仙俠、魔法。左上角隨時可切換（§2.3，不需確認），只影響「之後」新造武器的 skin 池／展覽牆背景／動物配色，已存在的任務保留原本分支跟 skin |
| Skin | 三個分支各 10 種，共 30 種全部可抽，皆為程序化 SVG 線稿圖示（卡片、任務詳情、鍛造儀式都看得到） |
| 分支背景 | 三種視覺主題：現代（金屬格線 + 冷光掃描）、仙俠（懸浮陣紋緩慢旋轉）、魔法（木紋 + 爐火餘光 + 邊緣龍頭裝飾） |
| 心情簽到 | 完整 5 種 mood + 密度／色溫映射 |
| 捕捉層 / 定義層 | 完整，含安全區規則、難度滑桿、EPIC 節點編輯 |
| 詞條庫 | 完整 50 個形容詞 + 7 個奇物彩蛋（彩色流動漸層） |
| EPIC / LEGACY | 完整，含品質計算、鍛造完成儀式（4 階段動畫）、次數門檻強化、重鑄外觀 |
| 回爐 / 靜置 | 完整，已完成的 EPIC 也可回爐 |
| 背景動物 | SDD 原始 10 種（景深、透明度、安靜組、輸入專注模式、2 幀走路/游動剪影，依分支變化配色），另加一種真實圖片素材的狗（`animal_pictures/dog/`，4 幀跑步循環）。預設啟用 4 種，其餘可於設定頁自行開啟 |
| 心情日誌回顧 | 月曆熱區呈現（§13.2），顏色 = 當天心情，未簽到的日子單純留白，不做任何缺席提示 |
| Spotify | 深層連結 + 依心情對應歌單設定 |

尚未實作：跨裝置資料同步（見下方「所有資料存於 localStorage」）。
